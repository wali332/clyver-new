...
export function Hero() {
  return (
    <section className="relative overflow-hidden border-b border-line bg-paper pt-[4.5rem]">
      ...
      <Container className="relative grid gap-8 py-12 sm:py-16 md:gap-10 md:py-20 lg:grid-cols-[minmax(0,1.1fr)_minmax(0,0.9fr)] lg:items-center lg:gap-14 lg:py-24">
        <div className="min-w-0 max-w-2xl motion-safe:animate-fade-up">
          <p className="mb-4 text-xs font-medium uppercase tracking-[0.2em] text-muted sm:mb-5">
            Digital Product &amp; Software Studio
          </p>
          <h1 className="text-pretty-safe font-display text-[clamp(2rem,8.5vw,4.75rem)] leading-[1.04] tracking-[-0.04em] text-ink">
            <span className="block">Your business has a problem.</span>
            <span className="mt-2 block text-muted sm:mt-3">
              Let&apos;s build
              <span className="hidden sm:inline"> </span>
              <span className="block sm:inline">the solution.</span>
            </span>
          </h1>
          <p className="mt-5 max-w-xl text-base leading-relaxed text-muted sm:mt-6 md:mt-7 md:text-lg md:leading-8">
          ...
          <p className="mt-6 text-sm leading-relaxed text-muted sm:mt-7">{siteConfig.location}</p>
        </div>

        <div
          className="min-w-0 motion-safe:animate-fade-up motion-safe:[animation-delay:120ms] lg:-translate-y-14"
